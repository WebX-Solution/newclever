...
import { Star, Trophy, Building2, ThumbsUp, Briefcase } from 'lucide-react';
import Marquee from 'react-fast-marquee';
import CountUp from 'react-countup';

export default function TrustIndicators() {
  const stats = [
    { number: '5000+', label: 'Firmenwagen angekauft' },
    { number: '20000', label: 'Zufridene Kunden' },
    { number: '24/7', label: 'Express Service' },
    { number: '100%', label: 'Faire Bewertung' }
  ];

  const vehicleTypes = [
    'Dienstwagen', 'Leasingrückläufer', 'Flottenfahrzeuge', 'Geschäftswagen',
    'Poolfahrzeuge', 'Servicefahrzeuge', 'Außendienstfahrzeuge', 'Vorführwagen',
    'Managementfahrzeuge', 'Vertriebsfahrzeuge', 'Lieferfahrzeuge', 'Mietwagen'
  ];
      ...
      <div className="container mx-auto px-4 mb-16">
        <div className="flex flex-wrap justify-center gap-8">
          {[
            ...
            { icon: Building2, text: 'Firmenfahrzeug Experten' },
            { icon: ThumbsUp, text: 'Empfohlen von Unternehmen' }
          ].map((badge, index) => (
            <motion.div
              key={index}
