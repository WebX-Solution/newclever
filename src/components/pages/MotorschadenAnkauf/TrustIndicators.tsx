...
import { Shield, Star, Trophy, ThumbsUp, Wrench } from 'lucide-react';
import Marquee from 'react-fast-marquee';
import CountUp from 'react-countup';

export default function TrustIndicators() {
  const stats = [
    { number: '10000+', label: 'Motorschäden bewertet' },
  ...
  const damageTypes = [
    'Motorschaden', 'Kolbenschaden', 'Turboschaden', 'Motorsteuerung',
    'Kurbelwellenschaden', 'Ventilschaden', 'Nockenwellenschaden', 'Motorblockschaden',
    'Motordefekt', 'Motorschaden Export', 'Motorschaden Ankauf', 'Motorschaden Diagnose'
  ];
    ...
    <section className="py-20 bg-gradient-to-b from-gray-900 to-gray-800">
      ...
      <div className="container mx-auto px-4 mb-16">
        <div className="flex flex-wrap justify-center gap-8">
          {[
            { icon: Shield, text: 'Zertifizierter Ankauf' },
            { icon: Star, text: 'Top Bewertungen' },
            { icon: Trophy, text: 'Ausgezeichneter Service' },
            { icon: Wrench, text: 'Motorschaden Experten' },
            { icon: ThumbsUp, text: 'Empfohlen von Werkstätten' }
          ].map((badge, index) => (
            <motion.div
              key={index}
